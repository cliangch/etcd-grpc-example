...
const (
	// minimum time to give a conn to complete
...
var (
	...
	ErrClientConnClosing = status.Error(codes.Canceled, "grpc: the client conn is closing")
	// errConnDrain indicates that the conn starts to be drained and does not accept any new RPCs.
	errConnDrain = errors.New("grpc: the conn is drained")
	// errConnClosing indicates that the conn is closing.
	errConnClosing = errors.New("grpc: the conn is closing")
	// errConnUnavailable indicates that the conn is unavailable.
	errConnUnavailable = errors.New("grpc: the conn is unavailable")
...
var (
	...
	// information (e.g., oauth2 token) which requires secure conn on an insecure
	// conn.
	errTransportCredentialsMissing = errors.New("grpc: the credentials require transport level security (use grpc.WithTransportCredentials() to set)")
	// errCredentialsConflict indicates that grpc.WithTransportCredentials()
	// and grpc.WithInsecure() are both called for a conn.
	errCredentialsConflict = errors.New("grpc: transport credentials are set for an insecure conn (grpc.WithTransportCredentials() and grpc.WithInsecure() are both called)")
	// errNetworkIO indicates that the conn is down due to some network I/O error.
...
// RegisterChannelz turns on channelz transport.
// This is an EXPERIMENTAL API.
func RegisterChannelz() {
	channelz.TurnOn()
}

// DialOption configures how we set up the conn.
type DialOption func(*dialOptions)

// WithWaitForHandshake blocks until the initial settings frame is received from the
// server before assigning RPCs to the conn.
// Experimental API.
...
// WithInitialConnWindowSize returns a DialOption which sets the value for initial window size on a conn.
// The lower bound for window size is 64K and any value smaller than that will be ignored.
...
// WithDefaultCallOptions returns a DialOption which sets the default CallOptions for calls over the conn.
...
//
// The balancer cannot be overridden by balancer option specified by transport
// config.
//
// This is an EXPERIMENTAL API.
...
// WithServiceConfig returns a DialOption which has a channel to read the transport configuration.
//
// Deprecated: transport config should be received through name resolver, as specified here.
...
// when backing off after failed conn attempts.
func WithBackoffMaxDelay(md time.Duration) DialOption {
	return WithBackoffConfig(BackoffConfig{MaxDelay: md})
}

// WithBackoffConfig configures the dialer to use the provided backoff
// parameters after conn failures.
//
...
// conn is up. Without this, Dial returns immediately and connecting the server
...
// conn level security credentials (e.g., TLS/SSL).
...
// If f is true, and dialer returns a non-temporary error, gRPC will fail the conn to the network
...
// transport config provided by the resolver and provides a hint to the resolver
// to not fetch transport configs.
...
// Dial creates a client conn to the given target.
func Dial(target string, opts ...DialOption) (*ClientConn, error) {
	return DialContext(context.Background(), target, opts...)
}

// DialContext creates a client conn to the given target. By default, it's
// a non-blocking dial (the function won't wait for connections to be
// established, and connecting happens in the background). To make it a blocking
// dial, use WithBlock() dial option.
//
// In the non-blocking case, the ctx does not act against the conn. It
// only controls the setup steps.
//
// In the blocking case, ctx can be used to cancel or expire the pending
// conn. Once this function returns, the cancellation and expiration of
...
func DialContext(ctx context.Context, target string, opts ...DialOption) (conn *ClientConn, err error) {
	...
	if cc.dopts.scChan != nil {
		// Try to get an initial transport config.
		select {
	...
	if cc.dopts.scChan != nil && !scSet {
		// Blocking wait for the initial transport config.
		select {
		case sc, ok := <-cc.dopts.scChan:
			if ok {
				cc.sc = sc
...
// ClientConn represents a client conn to an RPC server.
...
// If there's an exact match for input method (i.e. /transport/method), we return
// the corresponding MethodConfig.
// If there isn't an exact match for the input method, we look for the default config
// under the transport (i.e /transport/). If there is a default MethodConfig for
// the transport, we return it.
...
// handleServiceConfig parses the transport config string in JSON format to Go native
...
func (cc *ClientConn) handleServiceConfig(js string) error {
	...
	cc.scRaw = js
	cc.sc = sc
	if sc.LB != nil && *sc.LB != grpclbName { // "grpclb" is not a valid balancer option in transport config.
	...
	if envConfigStickinessOn {
		...
		// - stickinessMetadataKey field doesn't exist in transport config
...
// addrConn is a network conn to a given address.
type addrConn struct {
	...
	// connectDeadline is the time by which all conn
...
func (ac *addrConn) resetTransport() error {
	...
	for connectRetryNum := 0; ; connectRetryNum++ {
		...
		if ac.backoffDeadline.IsZero() {
			// This means either a successful HTTP2 conn was established
...
// createTransport creates a conn to one of the backends in addrs.
// It returns true if a conn was established.
