...
func (bw *balancerWrapper) lbWatcher() {
	...
	if notifyCh == nil {
		...
		if err != nil {
			grpclog.Warningf("Error creating conn to %v. Err: %v", a, err)
		} else {
			bw.mu.Lock()
			bw.conns[a] = sc
	...
	for addrs := range notifyCh {
		...
		if bw.pickfirst {
			...
			if oldSC == nil {
				...
				if err != nil {
					grpclog.Warningf("Error creating conn to %v. Err: %v", newAddrs, err)
				} else {
					bw.mu.Lock()
...
func (bw *balancerWrapper) Pick(ctx context.Context, opts balancer.PickOptions) (balancer.SubConn, func(balancer.DoneInfo), error) {
	...
	if bw.pickfirst {
		...
		if !ok && failfast {
			return nil, nil, status.Errorf(codes.Unavailable, "there is no conn available")
		}
...
// Idle and Shutdown are transitioned into by ClientConn; in the beginning of the conn
