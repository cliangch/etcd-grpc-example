...
func newHTTP2Client(connectCtx, ctx context.Context, addr TargetInfo, opts ConnectOptions, onSuccess func()) (_ ClientTransport, err error) {
	...
	// Any further errors will close the underlying conn
	...
	go func() {
		...
		// If it's a conn error, let reader goroutine handle it
...
func (t *http2Client) getCallAuthData(ctx context.Context, audience string, callHdr *CallHdr) (map[string]string, error) {
	...
	if callCreds := callHdr.Creds; callCreds != nil {
		if !t.isSecure && callCreds.RequireTransportSecurity() {
			return nil, streamErrorf(codes.Unauthenticated, "transport: cannot send secure credentials on an insecure conn")
		}
...
// keepalive running in a separate goroutune makes sure the conn is alive by sending pings.
