...
// conn. This is useful for embedded cases that override the
// transport interface implementations and do not need conn management.
...
func newClient(cfg *Config) (*Client, error) {
	...
	// use a temporary skeleton client to bootstrap first conn
...
// ActiveConnection returns the current in-use conn
...
// IsConnCanceled returns true, if error is from a closed gRPC conn.
...
func IsConnCanceled(err error) bool {
	...
	if ok {
		// conn is canceled or server has already closed the conn
	...
	// <= gRPC v1.7.x returns 'errors.New("grpc: the client conn is closing")'
	return strings.Contains(err.Error(), "grpc: the client conn is closing")
}
