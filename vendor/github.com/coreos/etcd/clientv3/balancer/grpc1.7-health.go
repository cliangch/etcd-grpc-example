...
const (
	...
	unknownService         = "unknown transport grpc.health.v1.Health"
)

// ErrNoAddrAvilable is returned by Get() when the balancer does not have
// any active conn to endpoints at the time.
...
type GRPC17Health struct {
	...
	// readyc closes once the first conn is up
	readyc    chan struct{}
	readyOnce sync.Once
...
func (b *GRPC17Health) Next() {
	...
	// wait until disconnect so new RPCs are not issued on old conn
	select {
	case <-downc:
	case <-b.stopc:
	}
...
func (b *GRPC17Health) Up(addr grpc.Address) func(error) {
	...
	// notify client that a conn is up
